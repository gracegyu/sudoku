...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:frame]))
    {
        ...
        tTextLabel.text = gettext(@"Achievement Unlocked", @"Achievemnt Unlocked Message");
        ...
        [tDetailLabel setMinimumScaleFactor:10.0f];
...
BOOL IsiPhoneX2(void)
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        int version = 0;
        if ([model length] >= 8)
            version = [[model substringWithRange:NSMakeRange(6,2)] intValue];
        DLog(@"model = %@, %d", model, version);
